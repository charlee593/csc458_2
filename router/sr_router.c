...
void process_arp_reply(struct sr_instance* sr, struct sr_arp_hdr* arp_hdr, struct sr_if* iface)
{
    ...
    if(req)
    {
        ...
        {
            struct sr_icmp_t0_hdr* icmp_hdr = (struct sr_icmp_t0_hdr*)(curr_packet_to_send->buf + sizeof(struct sr_ethernet_hdr) + sizeof(struct sr_ip_hdr));

        	if(icmp_hdr->icmp_type == icmp_type_echo_req)
        	{
                send_echo_reply(sr, curr_packet_to_send->buf, iface->name);
        	}
        	else{
