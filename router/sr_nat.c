...
void nat_send_icmp_t3(struct sr_instance* sr, sr_icmp_dest_unreachable_code_t icmpCode, sr_ip_hdr_t* originalPacketPtr)
{
	...
	/*   linkArpAndSendPacket(sr, (sr_ethernet_hdr_t*) replyPacket,
	   ...
	   if (arpEntry != NULL)
	   {
	      ...
	      sr_send_packet(sr, (uint8_t*) packet, sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t) + sizeof(sr_icmp_t3_hdr_t), icmpRoute->interface);
	   ...
	   else
	   {
	      ...
	      struct sr_arpreq* arpRequestPtr = sr_arpcache_queuereq(&sr->cache, icmpRoute->gw.s_addr,
